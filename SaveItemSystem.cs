...
public class SaveItemSystem //Skapar en klass med SaveItemSystem
{
      public void SaveItem(List<Item> items) //Sparar en lista av item-objekt till filen items.txt
      {
            ...
            {
                  lines.Add($"{item.OwnerUsername},{item.Name},{item.Description},{item.Tradeable}");  //Lägger till itemet med formatet linens med , mellan varje värde.
            }
      ...
      public List<Item> LoadItems() //Läser in item-objekt från filen items.txt och returnar ett item.
      {
            ...
            foreach (string line in lines) //Loopar alla rader i listan.
            {
                  ...
                  if (split.Length == 4) //Kontrollerar att varje rad är likamed 3.
                  {
                        string owner = split[0];
                        string name = split[1];
                        string description = split[2];
                        bool tradeable = bool.Parse(split[3]);
                        items.Add(new Item(owner, name, description, tradeable)); //Lägger till varje rad som uppfyllt dem 3 till ett item. Dessa fälten kan inte ta emot en tom sträng. Därför finns det inga items som inte uppfyller dessa kraven.
                  }
            }
