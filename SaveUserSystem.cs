namespace App;
public class SaveUserSystem //Sparar och läser in IUser Account från filen users.txt
{
      public void SaveUser(List<IUser> users) //Sparar en lista av IUser objektet av Account till filen users.txt.
      {
      ...
      public List<IUser> LoadUser() //Läser in IUser objektet Account från filen users.txt. och retunrerar en lista av users.
      {
            ...
            foreach (string line in lines) //Loopar varje rad i users.txt
            {
                  ...
                  if (split.Length == 3) // Kontrollerar att raden har exakt tre strängar.
                  {
                        string name = split[0];
